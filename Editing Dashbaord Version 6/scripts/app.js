    ...
    app.innerHTML = `
      ...
      <div class="content">
        ${(() => { try {
          ...
          let html = renderTodayTab();
          const weekKey = getWeekKey();
          const objsHtml = window._weekObjsHtml(state.data.weekObjectives, weekKey, true);
          if (objsHtml) {
            const marker = 'plan-week-banner';
            const idx = html.indexOf(marker);
            if (idx !== -1) {
              let depth = 0, i = idx;
              while (i < html.length) {
                if (html[i] === '<') {
                  if (html.startsWith('</div>', i)) { if (depth === 0) { i += 6; break; } depth--; }
                  else if (html.startsWith('<div', i)) depth++;
                }
                i++;
              }
              html = html.slice(0, i) + objsHtml + html.slice(i);
            }
          }
          return html;
