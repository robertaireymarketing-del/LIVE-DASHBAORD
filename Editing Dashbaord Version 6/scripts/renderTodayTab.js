export function renderTodayTab(deps) {
  const { state, BATCH_COLOURS, getTodayData, getIdentityLock, getMissionTargets, getProjectFronts, getTJMBatches, getLatestWeight, getLatestBodyFat, getLatestWeightDate, getLatestBodyFatDate, formatSyncLabel, getSettings, isSunday, getTodayDayKey, getWeekKey, getToday, getMonthStats, getMonthDaysRemaining, getMonthTargets, getDerivedTargetWeight, getCurrentLeanMass, getStartLeanMass, getStreak, renderInputCard } = deps;
        ...
        // Sunday target: use this week's Monday health sync as baseline (not today's rolling value)
        // Monday = day of week 1. Find most recent Monday date.
        const now = new Date();
        const dayOfWeek = now.getDay(); // 0=Sun, 1=Mon...
        const daysToMon = dayOfWeek === 0 ? 6 : dayOfWeek - 1;
        const mondayDate = new Date(now); mondayDate.setDate(now.getDate() - daysToMon);
        const mondayStr = mondayDate.toISOString().slice(0,10);
        // Find the health sync entry on or closest after Monday this week
        const thisWeekEntries = (state.healthData||[]).filter(h => h.date >= mondayStr).sort((a,b)=>a.date.localeCompare(b.date));
        const weekBaseEntry = thisWeekEntries[0]; // first reading of this week
        const weekBaseWeight = weekBaseEntry?.weight || currentWeight;
        const weekBaseBF = weekBaseEntry?.bodyFat || currentBF;
        // Weekly drop targets: 1.4lb weight, bfLossRate% BF — fixed for the whole week
        ...
        // Project fronts — unified daily view
        const DAY_NAMES = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
        const DAY_KEYS = ['sun','mon','tue','wed','thu','fri','sat'];
        const todayDayKey = getTodayDayKey();
        const weekKey = getWeekKey(); // Always current week — Sunday is still part of this week
  
        // Gather today's tasks — manual + batch steps + streams
        const allTodayTasks = [];
        ...
        // Batch steps scheduled for today
        ...
        // Livestreams scheduled for today
        ...
        const frontsSection = `
          ...
          <div class="plan-week-banner" style="background:linear-gradient(135deg,rgba(201,168,76,0.15),rgba(201,168,76,0.07));border:1.5px solid rgba(201,168,76,0.4);border-radius:14px;padding:16px 18px;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between;gap:12px;">
            <div>
              <div style="font-size:15px;font-weight:800;color:#C9A84C;margin-bottom:3px;">Plan Your Week</div>
              <div style="font-size:12px;color:rgba(255,255,255,0.4);">Add tasks, batch steps & livestreams to your days</div>
            </div>
            <button onclick="openDayPlanner()" style="background:#C9A84C;border:none;border-radius:10px;padding:14px 28px;color:#000;font-size:16px;font-weight:900;cursor:pointer;font-family:inherit;white-space:nowrap;letter-spacing:0.5px;box-shadow:0 0 0 3px rgba(201,168,76,0.3);">Open →</button>
          </div>
        ...
        // Habits + Sales (existing — keep at bottom)
        const habitsSection = `
        ...
        return identitySection + habitsSection + bodyStats + missionSection + frontsSection + batchesSection + `
