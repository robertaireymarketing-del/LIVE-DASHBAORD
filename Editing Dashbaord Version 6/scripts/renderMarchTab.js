  ...
  function getJournalWordRating(dateStr) {
    ...
    return                { label: 'LOW',       colour: '#e74c3c' };
        ...
        for (let day = 1; day <= daysInMonth; day++) {
          ...
          const slot1 = didRetention ? '🩸' : '·';
          const slot2 = didGym       ? '🏋️' : '·';
          const slot3 = didMeditate  ? '🧘' : '·';
          const slot4 = didLive      ? '📷' : ((dayData.sales||0)>0 ? '💰' : '·');
          ...
          const ratingHtml = journalRating ? `<span style="display:block;font-size:6.5px;font-weight:900;letter-spacing:0.3px;color:${journalRating.colour};opacity:0.95;line-height:1.2;margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:clip;">${journalRating.label}</span>` : '';
