...
export function initWeeklyTab() {
  ...
  window.weeklyStartEditWin = (i) => {
    ...
    const dayBtns = DAYS_SHORT.map((d, di) => {
      ...
      return '<button id="wkEditWinDay'+i+'_'+di+'" onclick="wkEditWinPickDay('+i+','+di+',this)" style="flex:1;padding:5px 2px;border-radius:7px;border:1px solid '+(active?'#1a1917':'#d8d5cc')+';background:'+(active?'#1a1917':'#f7f6f2')+';font-family:monospace;font-size:10px;font-weight:700;color:'+(active?'#fff':'#6b6860')+';cursor:pointer;">'+d+'</button>';
    }).join('');
    el.innerHTML = '<div style="flex:1;display:flex;flex-direction:column;gap:8px;">'
      + '<input id="wkEditWinInp'+i+'" class="wk-day-form-inp" value="'+wkEsc(win.text)+'" style="padding:4px 0 8px;border-bottom:1px solid #e5e3dc;width:100%;" />'
