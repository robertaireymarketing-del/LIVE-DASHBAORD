export function renderProgressTab(deps) {
  const { state, BATCH_COLOURS, getTodayData, getIdentityLock, getMissionTargets, getProjectFronts, getTJMBatches, getLatestWeight, getLatestBodyFat, getSettings, isSunday, getTodayDayKey, getWeekKey, getToday, getMonthStats, getMonthDaysRemaining, getMonthTargets, getDerivedTargetWeight, getCurrentLeanMass, getStartLeanMass } = deps;
        ...
        // Derive effective target based on mode
        ...
        if (targetMode === 'date' && settings.deadline) {
          // End date set → calculate what BF% they'll reach
          ...
          // BF target set → calculate end date
        ...
        // All sync entries sorted oldest→newest
        ...
        // Challenge start entry (closest to startDate)
        ...
        // Overall pace since challenge start
        ...
        // Weighted blend: 60% recent, 40% overall (only when both available)
        ...
        const actualBfPerWeek = effectivePace; // keep for downstream use
  
        // Expected weight at goal
        const expectedWeightAtGoal = +(currentLeanMass / (1 - settings.targetBodyFat / 100)).toFixed(1);
  
        // Weekly target weight (lose bfLossRate% BF per week, maintain lean mass)
        const weeklyTargetWeight = +(currentWeight - (currentWeight * bfLossRate / 100)).toFixed(1);
        // ──────────────────────────────────────────────────────────
  
        // --- WEEKLY CALORIE DEFICIT TRACKING (from health sync) ---
        const todayStr = new Date().toISOString().slice(0, 10);
        const weekStartCalc = new Date();
        const dayOfWeekCalc = weekStartCalc.getDay();
        const daysToMonCalc = dayOfWeekCalc === 0 ? 6 : dayOfWeekCalc - 1;
        weekStartCalc.setDate(weekStartCalc.getDate() - daysToMonCalc);
        const weekStartStr = weekStartCalc.toISOString().slice(0, 10);
        const weekHealthData = (state.healthData || []).filter(h => h.date >= weekStartStr && h.date <= todayStr);
        const daysLogged = weekHealthData.length;
        const totalCaloriesEaten = weekHealthData.reduce((s, h) => s + (h.calories || 0), 0);
        const totalBMR = weekHealthData.reduce((s, h) => s + (h.bmr || settings.defaultBmr), 0);
        const totalActiveCalories = weekHealthData.reduce((s, h) => s + (h.gymCalories || 0), 0);
        const totalBurn = totalBMR + totalActiveCalories;
        const weeklyDeficit = totalBurn - totalCaloriesEaten;
        const targetWeeklyDeficit = 3500 * (weightToLose > 0 ? Math.min(1.5, weightToLose) : 0.5); // ~500-750 kcal/day
        const dailyAvgDeficit = daysLogged > 0 ? Math.round(weeklyDeficit / daysLogged) : 0;
        const deficitOnTrack = weeklyDeficit >= targetWeeklyDeficit * 0.8;
  
        // --- WEEKLY & MONTHLY STEPS ---
        // THIS WEEK = Mon 00:00 to today (proper calendar week, not rolling 7-day)
        const weekStartDate = new Date(todayStr + 'T00:00:00');
        const dowToday = weekStartDate.getDay(); // 0=Sun
        ...
        // LAST WEEK same-point comparison: sum Mon–(Mon+daysBackToMon) of last week
        ...
        // LAST MONTH same-point comparison: same day-of-month range
        const stepsMonthDayOfMonth = parseInt(todayStr.slice(8, 10));
        const lastMonthYear = todayStr.slice(0, 7) === `${new Date().getFullYear()}-01`
          ? `${new Date().getFullYear() - 1}-12` : (() => { const d = new Date(todayStr.slice(0,7)+'-01'); d.setMonth(d.getMonth()-1); return d.toISOString().slice(0,7); })();
        const lastMonthSamePointStr = `${lastMonthYear}-${String(stepsMonthDayOfMonth).padStart(2,'0')}`;
        ...
        // --- WEIGHT & BF CHANGE WEEK/MONTH ---
        // Week: compare against last Sunday (start of week); Month: compare against 1st of month
        const todayDate = new Date();
        const dayOfWeek2 = todayDate.getDay(); // 0=Sun
        ...
        // Find closest entry on or before each reference date
        ...
        // Fall back to manual day entries if no health sync data
        ...
        return `
              ...
              <div style="margin-top:12px;margin-bottom:4px;">
                ...
                ` : `
                  ...
                  ${settings.targetBodyFat ? (() => {
                    ...
                    endDate.setDate(endDate.getDate() + Math.ceil(weeksNeeded * 7));
          ...
          <!-- GOAL PROJECTION CARD (navy command style) -->
          ...
          <!-- HERO STRIP: BF + WEIGHT (Design A layout, Design C colours) -->
          ...
          <!-- 4-WEEK PROJECTION -->
          ${(() => {
            const projBfRate = effectivePace;
            const projWtRate = effectiveWtPace;
  
            const rows = [1,2,3,4].map(w => {
              const actualBf = +(currentBF - projBfRate * w).toFixed(1);
              const actualWt = +(currentWeight - projWtRate * w).toFixed(1);
              const targetBf = +(currentBF - bfLossRate * w).toFixed(1);
              const targetWt = +(currentWeight - targetWtPace * w).toFixed(1);
              const onTrack = hasEnoughData ? (projBfRate >= bfLossRate * 0.8) : null;
            ...
            <div style="background:linear-gradient(145deg,#1a1a1a,#141414);border:1px solid rgba(255,255,255,0.08);border-radius:14px;overflow:hidden;margin-bottom:16px;box-shadow:0 4px 16px rgba(0,0,0,0.4);">
              <!-- Header -->
              <div style="display:grid;grid-template-columns:60px 1fr 1fr 64px;gap:0;padding:12px 14px;border-bottom:1px solid rgba(255,255,255,0.08);background:rgba(0,0,0,0.3);">
                <div></div>
              ...
              ${rows.map((r, i) => `
              <div style="display:grid;grid-template-columns:60px 1fr 1fr 64px;gap:0;padding:12px 14px;${i < 3 ? 'border-bottom:1px solid rgba(255,255,255,0.05);' : ''}align-items:center;">
                <div>
          ...
          <div class="section-title">This Week's Calorie Deficit</div>
          <div style="background:${weeklyDeficit > 0 ? 'rgba(46,204,113,0.08)' : 'rgba(231,76,60,0.08)'};border:1px solid ${weeklyDeficit > 0 ? 'rgba(46,204,113,0.25)' : 'rgba(231,76,60,0.25)'};border-radius:14px;padding:16px;margin-bottom:12px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
              <div>
                <div style="font-size:10px;font-weight:900;color:rgba(255,255,255,0.65);letter-spacing:1.5px;">DEFICIT SO FAR</div>
                <div style="font-size:26px;font-weight:700;color:${weeklyDeficit > 0 ? '#2ecc71' : '#e74c3c'};">${weeklyDeficit > 0 ? '-' : '+'}${Math.abs(Math.round(weeklyDeficit)).toLocaleString()} <span style="font-size:13px;font-weight:400;">kcal</span></div>
              </div>
              <div style="text-align:right;">
                <div style="font-size:10px;font-weight:900;color:rgba(255,255,255,0.65);letter-spacing:1.5px;">STATUS</div>
                <div style="font-size:14px;font-weight:700;color:${weeklyDeficit > 0 ? '#2ecc71' : '#e74c3c'};">${daysLogged === 0 ? '—' : weeklyDeficit > 0 ? (deficitOnTrack ? '⚡ IN DEFICIT' : '⚡ IN DEFICIT') : '⚠️ SURPLUS'}</div>
              </div>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;font-size:11px;">
              <div style="text-align:center;">
                <div style="color:rgba(255,255,255,0.6);font-size:10px;font-weight:800;margin-bottom:3px;letter-spacing:1px;">EATEN</div>
                <div style="color:#e74c3c;">${Math.round(totalCaloriesEaten).toLocaleString()} kcal</div>
              </div>
              <div style="text-align:center;">
                <div style="color:rgba(255,255,255,0.6);font-size:10px;font-weight:800;margin-bottom:3px;letter-spacing:1px;">BURNED</div>
                <div style="color:#2ecc71;">${Math.round(totalBurn).toLocaleString()} kcal</div>
              </div>
              <div style="text-align:center;">
                <div style="color:rgba(255,255,255,0.6);font-size:10px;font-weight:800;margin-bottom:3px;letter-spacing:1px;">DAILY AVG</div>
                <div style="color:${dailyAvgDeficit > 0 ? '#2ecc71' : '#e74c3c'};">${dailyAvgDeficit > 0 ? '-' : '+'}${Math.abs(dailyAvgDeficit).toLocaleString()}</div>
              </div>
            </div>
            <div style="font-size:11px;color:rgba(255,255,255,0.5);margin-top:8px;">${daysLogged} day${daysLogged !== 1 ? 's' : ''} synced this week · BMR + active calories tracked</div>
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:16px;">
            ...
            <div class="stat-card" style="padding:14px;text-align:center;">
              ...
              ${(() => {
                ...
                const monthGoal = 10000 * daysInCurrentMonth;
          ...
          ${(() => {
            ...
            const todayHealth = sorted[0]; // most recent sync entry
            ...
            const isToday = todayHealth.date === todayStr;
            const netCal = (todayHealth.calories||0) > 0 ? ((todayHealth.bmr||0) + (todayHealth.gymCalories||0) - (todayHealth.calories||0)) : null;
            return `
              <div class="section-title">Latest Sync <span style="font-size:10px;color:rgba(255,255,255,0.3);font-weight:400;margin-left:6px;">${isToday ? 'TODAY' : todayHealth.date} · AUTO · MIDNIGHT</span></div>
