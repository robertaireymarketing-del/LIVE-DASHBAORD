  ...
  const {
    ...
    VAULT_CAT_COLORS
      ...
      const capturePanel = capturing ? `
        <div style="background:rgba(212,175,55,0.06);border:1px solid rgba(212,175,55,0.2);border-radius:10px;padding:20px;margin-bottom:20px;">
          <div style="font-size:10px;letter-spacing:2px;color:#D4AF37;margin-bottom:16px;font-weight:600;">NEW CAPTURE</div>
          <input id="vault-title" placeholder="What's the idea?" style="width:100%;background:transparent;border:none;border-bottom:1px solid rgba(255,255,255,0.15);padding:8px 0;font-size:20px;color:#fff;outline:none;margin-bottom:20px;" class="vault-title-input"/>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
            <div>
              <div style="font-size:10px;letter-spacing:1px;color:rgba(255,255,255,0.4);margin-bottom:6px;">SOURCE</div>
            ...
            <div>
              <div style="font-size:10px;letter-spacing:1px;color:rgba(255,255,255,0.4);margin-bottom:6px;">CATEGORY</div>
          ...
          <div style="margin-bottom:14px;">
            <div style="font-size:10px;letter-spacing:1px;color:rgba(255,255,255,0.4);margin-bottom:6px;">WHAT'S THE SPARK?</div>
          ...
          <div style="background:rgba(126,184,201,0.07);border:1px solid rgba(126,184,201,0.2);border-radius:6px;padding:12px;margin-bottom:14px;" class="vault-action-box">
            ...
            <input id="vault-action" placeholder="e.g. Test as TikTok hook, delegate to VA, add to strategy doc..." style="width:100%;background:transparent;border:none;color:#fff;padding:4px 0;font-size:13px;outline:none;"/>
          </div>
          <div style="margin-bottom:18px;">
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;" onclick="vaultToggleVA()">
              ...
              <span style="font-size:12px;color:rgba(255,255,255,0.5);letter-spacing:1px;">DELEGATE TO VA</span>
            </label>
          ...
          <div style="display:flex;gap:10px;align-items:center;">
            <button onclick="vaultSaveIdea()" style="background:#D4AF37;color:#000;border:none;border-radius:6px;padding:10px 22px;font-size:13px;font-weight:700;letter-spacing:1px;cursor:pointer;">VAULT IT</button>
            <button onclick="vaultCancelCapture()" style="background:transparent;color:rgba(255,255,255,0.4);border:1px solid rgba(255,255,255,0.1);border-radius:6px;padding:10px 18px;font-size:12px;cursor:pointer;">Cancel</button>
          </div>
        </div>` :
        `<button onclick="vaultStartCapture()" class="vault-capture-btn" style="width:100%;padding:16px;background:rgba(212,175,55,0.06);border:1px dashed rgba(212,175,55,0.3);border-radius:8px;color:#D4AF37;font-size:14px;font-style:italic;cursor:pointer;margin-bottom:20px;text-align:left;">
          + Capture an idea...
        </button>`;

      const stageTabs = `<div style="display:flex;gap:2px;margin-bottom:20px;border-bottom:1px solid rgba(255,255,255,0.07);padding-bottom:0;">
        ${VAULT_STAGES.map(s => {
          const cnt = ideas.filter(i => i.stage === s).length;
          const active = activeStage === s;
          const col = VAULT_STAGE_COLORS[s];
          return `<button onclick="setVaultStage('${s}')" class="vault-stage-btn" style="background:transparent;border:none;border-bottom:2px solid ${active ? col : 'transparent'};padding:10px 12px 12px;color:${active ? col : 'rgba(255,255,255,0.3)'};font-size:12px;font-weight:600;letter-spacing:1px;cursor:pointer;margin-bottom:-1px;">
            ${s.toUpperCase()} <span style="background:${active ? 'rgba(255,255,255,0.08)' : 'transparent'};border-radius:10px;padding:1px 6px;font-size:10px;">${cnt}</span>
          </button>`;
        }).join('')}
      </div>`;
      ...
      const ideaCards = filtered.length === 0
        ...
        : filtered.map(idea => {
            ...
            return `
              <div class="vault-idea-card" style="background:rgba(255,255,255,0.03);border:1px solid ${isExpanded ? 'rgba(212,175,55,0.2)' : 'rgba(255,255,255,0.08)'};border-left:3px solid ${col};border-radius:8px;margin-bottom:8px;overflow:hidden;">
                <div onclick="toggleVaultExpand('${idea.id}')" style="padding:14px 16px;cursor:pointer;display:flex;align-items:center;gap:12px;">
                  <div style="flex:1;min-width:0;">
                    ...
                    <div style="display:flex;gap:10px;font-size:11px;">
                      ...
                      <span class="vault-idea-source">${idea.source}</span>
                      <span class="vault-idea-time">${vaultTimeAgo(idea.createdAt)}</span>
                    </div>
                  </div>
                  <div style="display:flex;align-items:center;gap:8px;">
                    <button class="vault-delete-quick-btn" onclick="event.stopPropagation();if(confirm('Delete this idea?'))deleteVaultIdea('${idea.id}')" title="Delete idea">✕</button>
                ...
                ${isExpanded ? `
                  <div style="padding:0 16px 16px;border-top:1px solid rgba(255,255,255,0.06);">
                    ${idea.spark ? `<div style="margin-top:14px;margin-bottom:12px;">
                      ...
                      <p class="vault-spark-text" style="margin:0;font-size:13px;line-height:1.6;font-style:italic;">${idea.spark}</p>
                    </div>` : ''}
                    ${idea.action ? `<div style="background:rgba(126,184,201,0.07);border:1px solid rgba(126,184,201,0.15);border-radius:6px;padding:10px 12px;margin-bottom:12px;">
                      ...
                      <p class="vault-action-text" style="margin:0;font-size:13px;">${idea.action}</p>
                    </div>` : ''}
                    ...
                    <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
                      ...
                      ${VAULT_STAGES.filter(s => s !== idea.stage).map(s => `<button class="vault-stage-move-btn" onclick="moveVaultStage('${idea.id}','${s}')" style="color:${VAULT_STAGE_COLORS[s]};">${s}</button>`).join('')}
                    </div>
                  </div>` : ''}
              </div>`;
          }).join('');
      ...
      return `
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px;">
          ...
          <div style="text-align:right;">
            <div class="vault-total-count">${ideas.length} total</div>
